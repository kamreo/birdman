...
public class ItemGrid : MonoBehaviour
{
    ...
    private InventoryGridItem[,] inventoryItemSlots;
    ...
    {
        inventoryItemSlots = new InventoryGridItem[gridSize.x, gridSize.y];
    ...
    public virtual InventoryGridItem PickUpItem(Vector2Int positionOnGrid)
    {
        ...
        if (positionOnGrid.x >= inventoryItemSlots.GetLength(0) || positionOnGrid.y >= inventoryItemSlots.GetLength(1))
            return null;

        InventoryGridItem returnItem = inventoryItemSlots[positionOnGrid.x, positionOnGrid.y];
        if (returnItem == null)
        {
            return null;
        }
    ...
    public virtual InventoryGridItem GetItem(Vector2Int positionOnGrid)
    {
        ...
        return inventoryItemSlots[positionOnGrid.x, positionOnGrid.y];
    }
    ...
    protected virtual void CleanGridReference(InventoryGridItem item)
    {
        ...
        for (int i = 0; i < item.itemData.size.x; i++)
        {
            ...
            {
                inventoryItemSlots[item.onGridPosition.x + i, item.onGridPosition.y + j] = null;
            }
        }
    }
    ...
    public virtual void PlaceItemInInventorySlot(InventoryGridItem item, Vector2Int itemPosition)
    {
        ...
        for (int i = 0; i < item.itemData.size.x; i++)
        {
            for (int j = 0; j < item.itemData.size.y; j++)
            {
                inventoryItemSlots[itemPosition.x + i, itemPosition.y + j] = item;
            }
        }
    ...
    protected virtual bool CheckAvailableSpace(Vector2Int position, Vector2Int size)
    {
        ...
        for (int i = 0; i < size.x; i++)
        {
            ...
            {
                if (inventoryItemSlots[position.x + i, position.y + j] != null)
                {
                    return false;
                }
            }
        ...
        for (int i = 0; i < size.x; i++)
        {
            for (int j = 0; j < size.y; j++)
            {
                if (inventoryItemSlots[position.x + i, position.y + j] != null)
                {
                    if (overlapItem == null)
                    {
                        overlapItem = inventoryItemSlots[position.x + i, position.y + j];
                    }
                    else
                    {
                        if (overlapItem != inventoryItemSlots[position.x + i, position.y + j])
                        {
                            return false;
                        }
                    }
